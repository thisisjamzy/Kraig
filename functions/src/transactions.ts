...
interface Contribution {
  ...
  signedAmount: number; // native to the account's own currency
}

function signedAmountOf(data: DocumentData): number {
  const amount = Number(data.amount) || 0;
  return data.direction === 'Inflow' ? amount : -amount;
}

function monthOf(data: DocumentData): string {
...
function contributionOf(data: DocumentData | undefined): Contribution | null {
 ...
 * not safe to assume.
 */
function isPureFieldCorrection(before: DocumentData, after: DocumentData): boolean {
  const beforeDate = before.date as Timestamp | undefined;
  const afterDate = after.date as Timestamp | undefined;
  return (
    before.amount === after.amount &&
    before.direction === after.direction &&
    before.accountId === after.accountId &&
    (before.categoryId ?? null) === (after.categoryId ?? null) &&
    (beforeDate?.isEqual(afterDate as Timestamp) ?? beforeDate === afterDate)
  );
}

export const onTransactionWrite = onDocumentWritten('users/{uid}/transactions/{transactionId}', async (event) => {
  ...
  if (before && after && isPureFieldCorrection(before, after)) return;
  ...
  const defaultCurrency = await getDefaultCurrency(uid);
  const rates = await getExchangeRates(uid);
  const now = new Date();
  ...
  for (const [contribution, sign] of [
    ...
    const currency = await currencyOfAccount(uid, contribution.accountId, defaultCurrency);
    const nativeDelta = sign * contribution.signedAmount;
    const convertedDelta = convert(nativeDelta, currency, defaultCurrency, rates);
...
function applyDelta(
  ...
  batch.update(userDoc.collection('accounts').doc(c.accountId), {
    currentBalance: FieldValue.increment(nativeDelta),
  });
  ...
  const monthUpdate: Record<string, unknown> = {
    ...
    transactionCount: FieldValue.increment(countDelta),
  ...
  if (c.categoryId) {
    ...
    if (convertedDelta !== 0) {
      ...
      monthUpdate.perCategorySpend = { [c.categoryId]: FieldValue.increment(expense - income) };
    }
    monthUpdate.perCategoryCount = { [c.categoryId]: FieldValue.increment(countDelta) };
  }
  batch.set(userDoc.collection('statsMonthly').doc(c.month), monthUpdate, { merge: true });

  const homeUpdate: Record<string, unknown> = {
    totalBalanceBase: FieldValue.increment(convertedDelta),
    lastUpdated: FieldValue.serverTimestamp(),
  };
  if (c.month === currentMonth) {
    homeUpdate.thisMonthIncome = FieldValue.increment(income);
    homeUpdate.thisMonthExpense = FieldValue.increment(expense);
  }
