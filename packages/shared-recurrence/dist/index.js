...
function withinEndCondition(rule, occurrenceNumber, date) {
...
function periodicOccurrencesInMonth(anchor, stepDays, year, month) {
...
function ruleAppliesToMonth(rule, year, month) {
    ...
    if (!occ)
        return null;
    const monthStart = new Date(year, month - 1, 1);
    if (!withinEndCondition(rule, occ.occurrenceIndex, monthStart))
        return null;
    return occ;
}
/**
 ...
 */
function nextOccurrenceOnOrAfter(rule, from, until) {
    ...
    for (let occ = 0; occ < 600; occ++) {
        ...
        if (rule.frequency === 'Daily') {
            ...
            const stepMonths = rule.frequency === 'Monthly'
                ? interval
